...
class BlogEditForm(forms.ModelForm):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        if kwargs['instance'].approved:
            self.fields[
                'feedurl'].help_text = "Note that changing the feed URL will disable the blog pending new moderation"
            self.fields[
                'authorfilter'].help_text = "Note that changing the author filter will disable the blog pending new " \
                                            "moderation "
...
class ModerateRejectForm(forms.Form):
    ...
    modsonly = forms.BooleanField(required=False, label="Moderators only",
                                  help_text="Should message be sent only to moderators, and not to the submitter (for "
                                            "spam submissions mainly)")
